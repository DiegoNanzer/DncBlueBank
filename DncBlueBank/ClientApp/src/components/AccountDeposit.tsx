...
import { API_ACCOUNT_DEPOSIT, API_ACCOUNT } from '../constants';
...
export const AccountDeposit: React.FC = () => {
    ...
    return <div className="container myContainer">
        <h1>Deposit</h1>
        {
            loading ? <Spinner color="dark" />
                : <AccountDepositForm account={account} />
        }
    </div>
}
...
const AccountDepositForm: React.FC<{ account: IAccountModel }> =
    ({ account }) => {
        const { register, handleSubmit, errors, } = useForm<IDeposit>(
        ...
        const onSubmit = handleSubmit(async (data) => {
            ...
            const response = await fetch(API_ACCOUNT_DEPOSIT, requestOptions);
